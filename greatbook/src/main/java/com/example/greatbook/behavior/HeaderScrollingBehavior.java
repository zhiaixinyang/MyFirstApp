package com.example.greatbook.behavior;
 ...
 */
public class HeaderScrollingBehavior extends CoordinatorLayout.Behavior<SwipeRefreshLayout> {
    ...
    @Override
    public boolean onLayoutChild(CoordinatorLayout parent, SwipeRefreshLayout child, int layoutDirection) {
    ...
    @Override
    public boolean onStartNestedScroll(CoordinatorLayout coordinatorLayout, SwipeRefreshLayout child, View directTargetChild, View target, int nestedScrollAxes) {
        return (nestedScrollAxes & ViewCompat.SCROLL_AXIS_VERTICAL) != 0;
    }

    @Override
    public void onNestedScrollAccepted(CoordinatorLayout coordinatorLayout, SwipeRefreshLayout child, View directTargetChild, View target, int nestedScrollAxes) {
    ...
    @Override
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, SwipeRefreshLayout child, View target, int dx, int dy, int[] consumed) {
        if (dy < 0) {
            return;
        }
    ...
    @Override
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, SwipeRefreshLayout child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        if (dyUnconsumed > 0) {
            return;
        }
    ...
    @Override
    public boolean onNestedPreFling(CoordinatorLayout coordinatorLayout, SwipeRefreshLayout child, View target, float velocityX, float velocityY) {
        return onUserStopDragging(velocityY);
    }

    @Override
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, SwipeRefreshLayout child, View target) {
        if (!isScrolling) {
    ...
    private float getDependentViewCollapsedHeight() {
        return getDependentView().getResources().getDimension(R.dimen.behavior_collapsed_header_height);
    }
